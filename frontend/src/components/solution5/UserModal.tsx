    ...
    <>
      ...
      <Modal onClick={() => setShowModal(false)}>
        <ModalContent onClick={(e) => e.stopPropagation()}>
                ...
                <ProfilePhotoWrapper>
                  {modalMode === 'view' ? (
                    getUserImage(selectedUser || formData as User) ? (
                      <ProfilePhoto src={getUserImage(selectedUser || formData as User)} alt={(selectedUser?.display_name || formData.display_name || '')} />
                    ) : (
                    ...
                    <ProfilePhoto src={selectedFile ? URL.createObjectURL(selectedFile) : getUserImage(formData as User)} alt={formData.display_name || ''} />
                  ) : (
                    <ProfilePhotoPlaceholder>
                      {(formData.first_name && formData.last_name) ? (
                        <span style={{ fontSize: 32, fontWeight: 600 }}>{formData.first_name[0]?.toUpperCase()}{formData.last_name[0]?.toUpperCase()}</span>
                      ) : <UserIcon size={40} />}
                    </ProfilePhotoPlaceholder>
                  )}
                  {modalMode !== 'view' && (
                    <FileInput style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', minHeight: 40 }}>
                      {selectedFile || formData.avatar_url ? (
                        <button type="button" onClick={handleRemoveFile} style={{ background: '#dc2626', border: 'none', color: '#fff', cursor: 'pointer', padding: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%', width: 32, height: 32, marginLeft: 'auto', marginRight: 0, boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }} title={t('userpage.removePhoto') || 'Remove'}><X size={20} color="#fff" /></button>
                      ) : (
                        <><input type="file" id="avatar_url" accept="image/*" onChange={handleFileChange} /><FileLabel htmlFor="avatar_url" style={{ marginLeft: 'auto', marginRight: 0 }}><Upload size={16} /></FileLabel></>
                      )}
                    </FileInput>
                  )}
                </ProfilePhotoWrapper>
                ...
                <FormRow>
                  <FormGroup>
                    ...
                    <Select name="department" value={formData.department || ''} onChange={handleInputChange} disabled={modalMode === 'view'}>
                      ...
                      <option value="IT">{t('userpage.departmentIT')}</option><option value="Human Resources">{t('userpage.departmentHR')}</option>
                      <option value="Finance">{t('userpage.departmentFinance')}</option><option value="Marketing">{t('userpage.departmentMarketing')}</option>
                    </Select>
                  </FormGroup>
                  <FormGroup>
                    ...
                    <Select name="role" value={formData.role || ''} onChange={handleInputChange} disabled={modalMode === 'view'}>
                      ...
                      <option value="Admin">{t('userpage.roleAdmin')}</option><option value="Manager">{t('userpage.roleManager')}</option>
                      <option value="Staff">{t('userpage.roleStaff')}</option><option value="Guest">{t('userpage.roleGuest')}</option>
                    </Select>
                  </FormGroup>
                </FormRow>
              </FormSection>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', borderTop: '1px solid #eee', padding: '12px 20px', marginTop: 16 }}>
            <div style={{ fontSize: 13, color: '#888', whiteSpace: 'nowrap' }}>
              {modalMode !== 'create' && (
                <div style={{ display: 'flex', gap: 40, alignItems: 'center' }}>
                  <span>Created: {formatDateTime(userForMeta.created_at)}</span>
                  <span>Updated: {formatDateTime(userForMeta.updated_at)}</span>
                </div>
              )}
            </div>
            <ButtonGroup>
